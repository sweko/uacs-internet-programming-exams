...
export class BandDetailComponent implements OnInit {
  ...
  getSimilarBands(): void {
    if (this.band && this.band.genre) {
      this.bandsService.getSimilarBands(this.band.genre).subscribe((bands: Band[]) => {
  ...
  calculateEndYear(): string {
    if (this.band && this.band.albums) {
      const latestAlbum = this.band.albums.reduce((latest, current) => {
