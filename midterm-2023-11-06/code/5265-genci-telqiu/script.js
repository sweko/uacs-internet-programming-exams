var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __spreadArray = (this && this.__spreadArray) || function (to, from, pack) {
    if (pack || arguments.length === 2) for (var i = 0, l = from.length, ar; i < l; i++) {
        if (ar || !(i in from)) {
            if (!ar) ar = Array.prototype.slice.call(from, 0, i);
            ar[i] = from[i];
        }
    }
    return to.concat(ar || Array.prototype.slice.call(from));
};
var _this = this;
var loadData = function () { return __awaiter(_this, void 0, void 0, function () {
    var datauri, response, data;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                datauri = "https://raw.githubusercontent.com/sweko/uacs-internet-programming-exams/main/midterm-2023-11-06/data/bands.json";
                return [4 /*yield*/, fetch(datauri)];
            case 1:
                response = _a.sent();
                if (!response.ok) {
                    throw new Error("The data is not there");
                }
                return [4 /*yield*/, response.json()];
            case 2:
                data = _a.sent();
                return [2 /*return*/, data.metalBands];
        }
    });
}); };
var displayBands = function (metalbands) {
    var container = document.getElementById("band-container");
    container.innerHTML = "";
    for (var _i = 0, metalbands_1 = metalbands; _i < metalbands_1.length; _i++) {
        var metalband = metalbands_1[_i];
        var metalBandRow = generateMetalBandRow(metalband);
        container.append(metalBandRow);
    }
};
var generateMetalBandRow = function (metalband) {
    var row = document.createElement("div");
    row.classList.add("band-table");
    var idCell = document.createElement("div");
    idCell.classList.add("band-data", "band-id");
    idCell.innerHTML = metalband.id.toString();
    row.appendChild(idCell);
    var nameCell = document.createElement("div");
    nameCell.classList.add("band-data", "band-name");
    nameCell.innerHTML = metalband.name.toString();
    row.appendChild(nameCell);
    var formedCell = document.createElement("div");
    formedCell.classList.add("band-data", "band-formed");
    formedCell.innerHTML = metalband.formed.toString();
    row.appendChild(formedCell);
    var locationCell = document.createElement("div");
    locationCell.classList.add("band-data", "band-location");
    locationCell.innerHTML = metalband.location.toString();
    row.appendChild(locationCell);
    var countryCell = document.createElement("div");
    countryCell.classList.add("band-data", "band-country");
    countryCell.innerHTML = metalband.location.split(", ")[1];
    row.appendChild(countryCell);
    var membersCell = document.createElement("div");
    membersCell.classList.add("band-data", "band-members");
    membersCell.innerHTML = metalband.members.toString();
    row.appendChild(membersCell);
    var albumsCell = document.createElement("div");
    albumsCell.classList.add("band-data", "band-albums");
    albumsCell.innerHTML = "Has ".concat(metalband.albums.length.toString(), " albums");
    row.appendChild(albumsCell);
    return row;
};
loadData()
    .then(displayBands)
    .catch(function (error) { return console.error(error); });
function loadDataAndPopulateFilters() {
    return __awaiter(this, void 0, void 0, function () {
        var githubUrl, response, data, uniqueCountries, uniqueGenres, countrySelect_1, genreSelect_1, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 3, , 4]);
                    githubUrl = "https://raw.githubusercontent.com/sweko/uacs-internet-programming-exams/main/midterm-2023-11-06/data/bands.json";
                    return [4 /*yield*/, fetch(githubUrl)];
                case 1:
                    response = _a.sent();
                    if (!response.ok) {
                        throw new Error("Failed to load data");
                    }
                    return [4 /*yield*/, response.json()];
                case 2:
                    data = _a.sent();
                    uniqueCountries = __spreadArray([], new Set(data.metalBands.map(function (band) { return band.location.split(', ')[1]; })), true);
                    uniqueGenres = __spreadArray([], new Set(data.metalBands.map(function (band) { return band.genre; })), true);
                    countrySelect_1 = document.getElementById("country-filter-select");
                    uniqueCountries.forEach(function (country) {
                        var option = document.createElement("option");
                        option.value = country;
                        option.textContent = country;
                        countrySelect_1.appendChild(option);
                    });
                    genreSelect_1 = document.getElementById("genre-filter-select");
                    uniqueGenres.forEach(function (genre) {
                        var option = document.createElement("option");
                        option.value = genre;
                        option.textContent = genre;
                        genreSelect_1.appendChild(option);
                    });
                    return [3 /*break*/, 4];
                case 3:
                    error_1 = _a.sent();
                    console.error("Error loading data:", error_1);
                    return [3 /*break*/, 4];
                case 4: return [2 /*return*/];
            }
        });
    });
}
loadDataAndPopulateFilters();
