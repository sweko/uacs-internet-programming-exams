...
const yearSorter: BandSorter = (first, second) => first.formed - second.formed;
const locationSorter: BandSorter = (first, second) => first.name.localeCompare(second.name);
...
const applyFilter = () => {
    const bandNameElement = document.getElementById("name-filter") as HTMLInputElement;
...
const displayBands = (bands: Band[]) => {
    ...
    for (const band of bands) {
        ...
        container.appendChild(bandRow as any);
    }
}
